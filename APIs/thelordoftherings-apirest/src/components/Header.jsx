...
const Header = () => {
    ...
    useEffect(() => {
        if (context.mode.get === 2) {
            axios.get(`${baseURL}book`, { headers: { Authorization: `Bearer ${localStorage.getItem("token")}` }}).then(response => {
                context.data.set(response.data.docs);
            })
        } else if (context.mode.get === 3) {
            axios.get(`${baseURL}movie`, { headers: { Authorization: `Bearer ${localStorage.getItem("token")}` }}).then(response => {
                context.data.set(response.data.docs);
            })
        } else if (context.mode.get === 4) {
            axios.get(`${baseURL}character`, { headers: { Authorization: `Bearer ${localStorage.getItem("token")}` }}).then(response => {
                context.data.set(response.data.docs);
            })
        } else if (context.mode.get === 5) {
            axios.get(`${baseURL}quote`, { headers: { Authorization: `Bearer ${localStorage.getItem("token")}` }}).then(response => {
                context.data.set(response.data.docs);
            })
        }
        return () => {}
